...
UMAP_KWARGS = {"n_neighbors": 4, "min_dist": 0.1, "n_components": 2, "metric": "cosine", "init": "random"}
CLUSTERING_KWARGS = {
    "min_cluster_size": 3,
    "min_samples": 3,
...
def load_hf_embed_model(
    model_name: str, model_kwargs: Dict[str, Any], encode_kwargs: Dict[str, Any]
) -> HuggingFaceEmbeddings:
    """Wrap loading of HF Embedding model for streamlit caching.

    Parameters
    ----------
    model_name : str
        Model ID, passed to `HuggingFaceEmbeddings`
    model_kwargs : dict
        Model arguments, passed to `HuggingFaceEmbeddings`
    encode_kwargs : dict
        Encode arguments, passed to `HuggingFaceEmbeddings`
...
def load_llm(
    ...
    openai_api_key : str, optional
        OpenAI API key, passed to `VLLMOpenAI`, by default "EMPTY"
    openai_api_base : _type_, optional
        OpenAI API url, passed to `VLLMOpenAI`, by default "http://localhost:8000/v1"
    model_name : str, optional
        Name of the model being served, by default "mistralai/Mistral-7B-Instruct-v0.2"
...
with st.sidebar:
    ...
    st.header("What's your topic of interest ?", anchor=None, help=None, divider=False)
    st.write(
...
if run_clustering:
    ...
    with st.spinner("Embedding articles..."):
        ...
        df_embed = pd.DataFrame(
            data=embeddings, columns=[f"embed_{i}" for i in range(embeddings.shape[1])], index=df_data.index
        )
...
if run_clustering:
    ...
    with st.spinner("Summarizing clusters of articles..."):
        ...
        for k, idx in enumerate(clusters_list):
            ...
            # generate keywords
            keywords_pred = llm.predict(KEYWORD_PROMPT_TEMPLATE.format(summary))
            keywords_list = [s.strip() for s in keywords_pred.strip().split(",")]

            # store results
            df_analysis.loc[k, :] = np.array([[idx, title, keywords_list, summary]], dtype=object)
...
if "df_analysis" in st.session_state and "df_umap" in st.session_state:
    ...
    if cluster_choice:
        ...
        chain = (
            {
                "context": itemgetter("question") | retriever | llmu._combine_documents,
        ...
        if question:
            ...
            answer = chain.invoke({"question": question, "language": "English"})
    ...
    if st.session_state["cluster_choice"] is not None:
        ...
        if tts_button:
            ...
            tab2.audio(x, sample_rate=int(sr))
